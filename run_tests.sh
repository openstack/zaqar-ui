...
function run_tests {
  ...
  if [ $with_selenium -eq 0 -a $integration -eq 0 ]; then
      testopts="$testopts --exclude-dir=zaqar_ui/test/integration_tests"
  fi
...
function run_tests_all {
  echo "Running Zaqar UI tests"
  export NOSE_XUNIT_FILE=zaqar_ui/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='dashboard_nose_results.html'
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test zaqar_ui --settings=openstack_dashboard.test.settings $testopts
